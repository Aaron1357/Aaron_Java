...
public class B_Sorted {
	 ...
	 * IntStream, DoubleStream, LongStream은 요소를 오름차 순으로 정렬한다. 
	 */
	...
	Arrays.stream(new int[] {3, 5, 1 ,2 ,4}).sorted().forEach(value ->System.out.println(value));	
	   ...
	   //내림차순
	Arrays.stream(new int[] {3, 5, 1 ,2 ,4}).boxed().sorted(Comparator.reverseOrder()).forEach(value ->System.out.println(value));	
	}
	public void method2 () {
		List<String> names = Arrays.asList("김민수","구관원","임지우","박승환");
		
		//comparable 인터페이스구현 내용대로 정렬 
		names.stream().sorted().forEach(name -> System.out.println(name));	
		//comparable 인터페이스구현 내용반대로 정렬
	 ...
	 students.stream().sorted().forEach(value -> System.out.println(value));
	 
	    //내림차순
	 students.stream().sorted(Comparator.reverseOrder()).forEach(value -> System.out.println(value));
	}
